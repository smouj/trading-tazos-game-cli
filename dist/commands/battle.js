...
function simulateBattle(playerTazos, opponentTazos, seed) {
        ...
        if (hits) {
            ...
            if (rng() < captureChance) {
                if (isPlayerTurn) {
                    ...
                    if (idx >= 0) {
                        o.tazos.splice(idx, 1);
                        p.captures++;
                        log.push(chalk.green(`  CAPTURE! ${defName} joins your collection`));
                    }
                }
                else {
                    const idx = p.tazos.indexOf(defender);
                    if (idx >= 0) {
                        p.tazos.splice(idx, 1);
                        o.captures++;
                        log.push(chalk.red(`  LOST! ${defName} captured by opponent`));
                    }
                }
            }
    ...
    const winner = p.tazos.length > o.tazos.length ? "player"
